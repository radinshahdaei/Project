...
import Client.Model.Building.Barracks.Barracks;
import Client.Model.Building.*;
import Client.Model.Building.Storage.Storage;
import Client.Model.Building.WeaponMaker.WeaponMaker;
...
import Client.View.Game.GovernmentMenu;
...
public class BuildingMenuController {
    public static void dropBuilding(int x, int y, String type) {
        ...
        if (type.equalsIgnoreCase("killing pit")) {
            Client.Controller.GameMenuController.game.getMap().getTiles()[x][y].setHasKillingPit(true);
            InputOutput.output("Killing pit added");
            return;
        }
        if (type.equalsIgnoreCase("pitch ditch")) {
            Client.Controller.GameMenuController.game.getMap().getTiles()[x][y].setHasOil(true);
        ...
        Client.Controller.GameMenuController.game.getMap().getTiles()[x][y].setBuilding(building);
    ...
    public static void dropStairs(int x, int y) {
        int mapSize = Client.Controller.GameMenuController.mapSize;
        if (x < 0 || y < 0 || x >= mapSize || y >= mapSize) {
            InputOutput.output("Invalid coordinates!", 'e');
            return;
        }
        Tile tile = Client.Controller.GameMenuController.game.getMap().getTiles()[x][y];
    ...
    private static boolean checkSimpleErrorsOfDropBuilding(int x, int y) {
        ...
        if (Client.Controller.GameMenuController.game.getMap().getTiles()[x][y].getPeople().size() > 0) {
