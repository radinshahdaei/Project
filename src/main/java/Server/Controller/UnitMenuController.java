...
import Client.View.Game.UnitMenu;
...
public class UnitMenuController {
    public static void dropUnit(int x, int y, int count, String type) {
        ...
        if (militaryType == null) {
            ...
            InputOutput.output("This type of unit does not exists", 'd');
            return;
        }
        ...
        for (int i = 0; i < count; i++) {
            ...
            Client.Controller.GameMenuController.game.getMap().getTiles()[x][y].getPeople().add(militaryUnit);
        }
        InputOutput.output("units successfully dropped");
    }

    private static boolean checkSimpleErrorsOfDropUnit(int x, int y) {
        if (x >= Client.Controller.GameMenuController.mapSize || y >= Client.Controller.GameMenuController.mapSize) {
            InputOutput.output("Invalid coordinates");
            return true;
        }
        if (Client.Controller.GameMenuController.game.getMap().getTiles()[x][y].getBuilding() != null) {
            InputOutput.output("You cannot place units on top of buildings!");
            InputOutput.output("Invalid coordinates", 'd');
            return true;
        }
        if (Client.Controller.GameMenuController.game.getMap().getTiles()[x][y].getBuilding() != null) {
            InputOutput.output("You cannot place units on top of buildings!", 'd');
            return true;
        }
        return false;
    }
    ...
    private static boolean checkIfEnoughResourcesExist(MilitaryUnit militaryUnit, int count) {
        for (Resource resource : militaryUnit.getPrice()) {
            ...
            if (amount < resource.getCount() * count) {
                ...
                InputOutput.output("Not enough resources to buy this unit", 'd');
                return true;
            }
        }
        return false;
    ...
    public static void selectUnit(int x, int y, String type) {
        ...
        UnitMenu.userUnitInTile = getUserUnitInTile(Client.Controller.GameMenuController.game.getMap().getTiles()[x][y].getPeople(), type);
    ...
    private static boolean checkSimpleErrorsOfSelectUnit(int x, int y, String type) {
        ...
        ArrayList<MilitaryUnit> units = getUserUnitInTile(Client.Controller.GameMenuController.game.getMap().getTiles()[x][y].getPeople(), type);
        if (units.size() == 0) {
    ...
    public static void moveAllMilitaryUnits() {
        Tile[][] tiles = Client.Controller.GameMenuController.game.getMap().getTiles();
        Tile[][] newTiles = new Tile[Client.Controller.GameMenuController.mapSize][Client.Controller.GameMenuController.mapSize];
        for (int i = 0; i < Client.Controller.GameMenuController.mapSize; i++) {
            for (int j = 0; j < Client.Controller.GameMenuController.mapSize; j++) {
        ...
        Client.Controller.GameMenuController.game.getMap().setTiles(newTiles);
    }
    ...
    public static void attackArcher(int x, int y) {
        ...
        ArrayList<Person> peopleInTile = Client.Controller.GameMenuController.game.getMap().getTiles()[x][y].getPeople();
        Building building = Client.Controller.GameMenuController.game.getMap().getTiles()[x][y].getBuilding();
        for (MilitaryUnit militaryUnit : UnitMenu.userUnitInTile) {
            if (militaryUnit.getRange() > 0 && checkInRange(militaryUnit, x, y)) {
                ...
                if (Client.Controller.GameMenuController.game.getMap().getTiles()[x][y].isHasOil()) {
                    Client.Controller.GameMenuController.game.getMap().getTiles()[x][y].setHasOil(false);
                    Client.Controller.GameMenuController.game.getMap().getTiles()[x][y].setOnFire(true);
                }
            }
        }
    ...
    public static boolean patrolUnit(int x1, int x2, int y1, int y2) {
        if (x1 >= Client.Controller.GameMenuController.mapSize || y1 >= Client.Controller.GameMenuController.mapSize) {
            InputOutput.output("Invalid coordinates");
            return false;
        }
        if (x2 >= Client.Controller.GameMenuController.mapSize || y2 >= Client.Controller.GameMenuController.mapSize) {
            InputOutput.output("Invalid coordinates");
            return false;
        }
        for (MilitaryUnit militaryUnit : UnitMenu.userUnitInTile) {
    ...
    public static void cancelPatrol(int x, int y) {
        ...
        for (Person person : Client.Controller.GameMenuController.game.getMap().getTiles()[x][y].getPeople()) {
            if (person instanceof MilitaryUnit) {
                MilitaryUnit militaryUnit = (MilitaryUnit) person;
...
//        }
        ...
        while (UnitMenu.userUnitInTile.size() != 0) {
            UnitMenu.userUnitInTile = getUserUnitInTile(Client.Controller.GameMenuController.game.getMap().getTiles()[x][y].getPeople(), "engineer");
    ...
    public static void attackWithStatus() {
        for (Government government : Client.Controller.GameMenuController.game.getGovernments()) {
