...
import Client.View.Game.TradeGUI;
...
public class StoreGUI extends Application {
    ...
    public void start(Stage stage) throws Exception {
        ...
        Button buy = new Button("Buy 5 for 20 gold");
        buy.setMinWidth(140);
        buy.setOnAction(actionEvent -> {
            boolean result = false;
            showSuccess(result = StoreMenuController.buy(commodityToAction.get().resourceType.name, 5),"buy");
            if (result){
                for (BorderPane borderPane:commodities) {
                    if (commodityToAction.get().resourceType.name.equals(((Text) borderPane.getLeft()).getText())) {
                        Store.setStock(commodityToAction.get());
                        ((Text) borderPane.getCenter()).setText("Stock: " + String.valueOf(commodityToAction.get().stock));
                    }
                }
                stockpile.set((Storage) Game.currentGovernment.findBuildingByName("stockpile"));
                goldAmount.set(stockpile.get().getStoredResourceByType(ResourceType.GOLD).getCount());
        ...
        sell.setOnAction(actionEvent -> {
            boolean result = false;showSuccess(result = StoreMenuController.sell(commodityToAction.get().resourceType.name,5),"sell");
            if (result){
                for (BorderPane borderPane:commodities) {
                    if (commodityToAction.get().resourceType.name.equals(((Text) borderPane.getLeft()).getText())) {
                        Store.setStock(commodityToAction.get());
            ...
            stockpile.set((Storage) Game.currentGovernment.findBuildingByName("stockpile"));goldAmount.set(stockpile.get().getStoredResourceByType(ResourceType.GOLD).getCount());
            if (commodityToAction.get() != null) {
                label.setText("Selected commodity: "+ commodityToAction.get().resourceType.name + "\t\tGold: "+goldAmount);
            }
        });
